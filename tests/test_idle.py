...
class _FakeCW:
    ...
    def get_metric_data(self, MetricDataQueries, **kw) -> dict:
        ...
        for q in MetricDataQueries:
            ...
            results.append({"Id": q["Id"], "Values": self._by.get(iid, {}).get(metric, [])})
...
def test_only_low_cpu_low_network_instances_are_flagged():
    ...
    recs = IdleCollector(_FakeProvider(ec2, cw)).recommendations(None)
...
def test_idle_event_has_no_savings_clause():
    ...
    recs = IdleCollector(_FakeProvider(_FakeEC2(["i-idle"]), cw)).recommendations(None)
...
def test_no_instances_no_calls():
    recs = IdleCollector(_FakeProvider(_FakeEC2([]), _FakeCW({}))).recommendations(None)
    assert recs == []
...
def test_tuning_raises_cpu_threshold():
    ...
    tuned = IdleCollector(provider, FinOpsTuning(idle_cpu_pct=50.0))
